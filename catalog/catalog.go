package main

import (
	"encoding/json"
	"fmt"
	"html/template"
	"io/ioutil"
	"net/http"
	"os"
	"github.com/gorilla/mux"
)

type Product struct {
	Uuid    string  `json:"uuid"`
	Product string  `json:"product"`
	Price   float32 `json:"price,string"`
}

type Products struct {
	Products []Product `json:"products"`
}

// variavel de ambiente da URL para conectar com o microserviço de produtos
var productsUrl string

// metodo inicia antes do main 
func init() {
	productsUrl = os.Getenv("PRODUCT_URL")
}

func loadPrducts() []Product {
	// requisição para pegar produtos 
	response, err := http.Get(productsUrl + "/products")

	// tratamento de erro, caso exista 
	if err != nil {
		fmt.Println("Erro de HTTP")
	}

	// ler o corpo da resposta da requisição 
	data, _ := ioutil.ReadAll(response.Body)

	var products Products
	json.Unmarshal(data, &products)

	return products.Products
}

func main() {
	// criar roteamento 
	r := mux.NewRouter()

	// rotas 
	r.HandleFunc("/", ListProducts)
	r.HandleFunc("/product/{id}", ShowProduct)

	// porta do servidor 
	http.ListenAndServe(":8080", r)
}

func ListProducts(w http.ResponseWriter, r *http.Request) {
	products := loadPrducts()
	// templete engine do go 
	// indicar o arquivo html processado 
	t := template.Must(template.ParseFiles("templates/catalog.html"))

	t.Execute(w, products)
}

func ShowProduct(w http.ResponseWriter, r *http.Request) {
	
	vars := mux.Vars(r)

	// fazendo rrequisição
	response, err := http.Get(productsUrl + "/product/" + vars["id"])
	if err != nil {
		fmt.Printf("The HTTP request failed with error %s\n", err)
	}
	data, _ := ioutil.ReadAll(response.Body)

	var product Product
	json.Unmarshal(data, &product)

	t := template.Must(template.ParseFiles("templates/view.html"))
	t.Execute(w, product)
}

// comando para gerenciar os modulos do go
// $ go mod init catalog 
